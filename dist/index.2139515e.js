...
Array.from(inputData).map((input)=>{
    input.addEventListener("change", async (e)=>{
        let target = e.target;
        if (target) {
            if (target.name === "day") day = parseInt(target.value);
            else if (target.name === "month") month = parseInt(target.value) - 1;
            else if (target.name === "year") year = parseInt(target.value);
        }
